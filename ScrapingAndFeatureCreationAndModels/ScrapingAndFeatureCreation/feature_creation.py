...
def create_lower_upper_bands(stock):
    ...
    for i, row in stock.iterrows():
        maxv = stock.loc[i:]["Close Price"].max()
        minv = stock.loc[i:]["Close Price"].min()
...
def perform_operation(security_code):
    """
    Creates GR Stock File for the given company

    Parameters
    ----------

    security_code : security_code of the company
    ...
    try:
        ...
        stock_df = add_risk_free_column(stock_df, riskfreerate_df)
