...
def intial_run():
    ...
    for security_code in sp500companies:
        try:
            lbresult = run_companies_lb(security_code, columns_to_predict[8])
            ubresult = run_companies_ub(security_code, columns_to_predict[9])
    ...
    try:
        if result != []:
            ...
            resultdf.to_csv(os.path.join(os.getcwd(), "Data","next_360_days.csv"), index=None)
    except:
        pass
